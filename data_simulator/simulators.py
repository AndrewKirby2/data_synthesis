...
def simulator6d_halved(x):
    ...
    x3, y3
        Gaussian noise is added
        Maximum change is halved and isolated turbine CT* is removed
    ...
    CT* : float
        The value of the turbine thrust coefficient
    """
    return 0.025*np.exp(-x[0]/10)*np.exp(-x[1]**2/(10+0.2*x[0])) \
        - 0.2*np.exp(-x[0]/10)*np.exp(-x[1]**2/(0.5+0.2*x[0])) \
        + 0.025*np.exp(-x[2]/10)*np.exp(-x[3]**2/(10+0.2*x[2])) \
